...
# --- final_dataset_max_theta         <- this is the dataset of all participants with maximum theta power and ITF peaks
# --- final_dataset_max_alpha         <- this is the dataset of all participants with maximum alpha power and IAF peaks

# Use the following dataset loaded later in the script
# --- THRIVE CCDT                     <- this is the dprime readouts file
...
# --- merged_data_n20?                <- this is the dataset with participants with detectable ITF peaks during encoding and retrieval
...
final_dataset_max_theta <- file.path(main_directory, "final_dataset_max_theta.csv")

# Load the dataset
final_dataset_max_theta <- read.csv(final_dataset_max_theta)

# Full dataset - theta
descriptives_all <- final_dataset_max_theta %>%
  group_by(Condition) %>%
  summarise(
...
# Define the dataset file path
final_dataset_max_alpha <- file.path(main_directory, "final_dataset_max_alpha.csv")

# Load the dataset
final_dataset_max_alpha <- read.csv(final_dataset_max_alpha)

descriptives_all <- final_dataset_max_alpha %>%
  group_by(Condition) %>%
  summarise(
    mean_frequency = mean(Frequency, na.rm = TRUE),
    median_frequency = median(Frequency, na.rm = TRUE),
...
ggplot(final_dataset_max_theta, aes(x = Condition, y = Frequency)) +
...
ggplot(final_dataset_exclude_8446_participants, aes(x = Condition, y = Frequency)) +
  ...
  geom_signif(comparisons = list(c("encoding","er"),c("encoding", "retr"),c("er","lr"),c("er","retr")),
              annotations = c("*"," "," "," "),
...
ggplot(final_dataset_max_theta, aes(x = Condition, y = Power)) +
  ...
  # Significance annotations
  geom_signif(comparisons = list(c("encoding","er"),c("encoding","lr"),c("lr","retr"),c("er","retr")),
              annotations = c("***"," "," ","**"),
...
# ---- Plot A: Theta Power ----
plot_a <- ggplot(final_dataset_max_theta, aes(x = Condition, y = Power)) +
  ...
  geom_signif(
    comparisons = list(c("encoding","er"), c("encoding","lr"), c("lr","retr"), c("er","retr")),
    annotations = c("***", " ", " ", "**"),
  ...
  scale_x_discrete(labels = c("encoding" = "encoding", "er" = "early retention", "lr" = "late retention", "retr" = "response")) +
...
plot_b <- ggplot(final_dataset_max_theta, aes(x = Condition, y = Frequency)) +
  geom_violin(fill = "#cce5ff", color = NA, alpha = 0.6, trim = FALSE, width = 0.9, adjust = 1.2) +
...
plot_c <- ggplot(final_dataset_exclude_8446_participants, aes(x = Condition, y = Frequency)) +
  ...
  geom_jitter(position = position_jitter(width = 0.15), size = 1.5, alpha = 0.7, color = "dodgerblue") +
  geom_signif(
    comparisons = list(c("encoding","er"), c("encoding", "retr"), c("er","lr"), c("er","retr")),
    annotations = c("*", " ", " ", " "),
...
ggplot(final_dataset_max_alpha, aes(x = Condition, y = Frequency)) +
  ...
  # Significance annotations
  geom_signif(comparisons = list(c("encoding","er"),c("er","retr"),c("er","lr"),c("lr", "retr")),
              annotations = c("***"," ","**","*"),
...
ggplot(final_dataset_max_alpha, aes(x = Condition, y = Power)) +
...
# ---- Load Alpha Dataset ----
final_dataset_max_alpha <- read.csv(file.path(main_directory, "final_dataset_max_alpha.csv"))

# ---- Plot A: Alpha Power ----
plot_alpha_power <- ggplot(final_dataset_max_alpha, aes(x = Condition, y = Power)) +
  geom_violin(fill = "#ffdab3", color = NA, alpha = 0.6, trim = FALSE, width = 0.9, adjust = 1.2) +
  geom_boxplot(width = 0.15, outlier.shape = NA, fill = "#ffb84d", color = "#e67300", linewidth = 0.5, alpha = 0.85) +
  geom_jitter(position = position_jitter(width = 0.15), size = 1.5, alpha = 0.7, color = "tan1") +
  ...
  scale_x_discrete(labels = c("encoding" = "encoding", "er" = "early retention", 
                              "lr" = "late retention", "retr" = "response")) +
...
plot_alpha_freq <- ggplot(final_dataset_max_alpha, aes(x = Condition, y = Frequency)) +
  geom_violin(fill = "#cce5ff", color = NA, alpha = 0.6, trim = FALSE, width = 0.9, adjust = 1.2) +
  geom_boxplot(width = 0.15, outlier.shape = NA, fill = "#99ccff", color = "#3399ff", linewidth = 0.5, alpha = 0.85) +
  geom_jitter(position = position_jitter(width = 0.15), size = 1.5, alpha = 0.7, color = "dodgerblue") +
  geom_signif(comparisons = list(c("encoding", "er"), c("er", "retr"), c("er", "lr"), c("lr", "retr")),
...
file_path <- file.path(main_directory, "final_dataset_max_theta.csv")

# Load the dataset using the full file path
final_dataset_max_theta <- read.csv(file_path)

# ---- Friedman

# Assumption for RM-ANOVA
shapiro.test(final_dataset_max_theta$Power) # p<0.05, perform non-parametric Friedman
shapiro.test(final_dataset_max_theta$Frequency) # p<0.05, perform non-parametric Friedman

# Test differences across phases
friedman_power<-friedman.test(Power ~ Condition | File, data = final_dataset_max_theta)
print(friedman_power)

friedman_frequency<-friedman.test(Frequency ~ Condition | File, data = final_dataset_max_theta)
...
participants_with_8446_anywhere <- final_dataset_max_theta %>%
  filter(Frequency == 8.446) %>%
  distinct(File)

# Filter dataset to keep only participants who NEVER have 8.446Hz in any condition
final_dataset_no_8446_anywhere <- final_dataset_max_theta %>%
...
# Post-hoc testing
pairwise_frequency <-pairwise.wilcox.test(final_dataset_max_theta$Frequency,
                                          final_dataset_max_theta$Condition,
                                          paired = TRUE,
...
pairwise_power<-pairwise.wilcox.test(final_dataset_max_theta$Power,
                                     final_dataset_max_theta$Condition,
                                     paired = TRUE,
                                     p.adjust.method = "fdr") # use FDR for multiple testing
...
print(Zstat)
wilcox_effsize(final_dataset_max_theta,Power~Condition,paired=TRUE)
...
# Test differences across phases
friedman_power<-friedman.test(Power ~ Condition | File, data = final_dataset_max_alpha) # alpha
print(friedman_power)

friedman_frequency<-friedman.test(Frequency ~ Condition | File, data = final_dataset_max_alpha) # alpha
print(friedman_frequency)


pairwise_frequency <-pairwise.wilcox.test(final_dataset_max_alpha$Frequency,
                                          final_dataset_max_alpha$Condition,
                                          paired = TRUE,
                                          p.adjust.method = "fdr") # use FDR for multiple testing
print(pairwise_frequency)
...
Zstat<-qnorm(pairwise_frequency$p.value/2)
print(Zstat)
wilcox_effsize(final_dataset_max_alpha,Frequency~Condition,paired=TRUE)

# Extra t-statistics

get_wilcoxon_T_stats(
  data = final_dataset_max_alpha,
  variable = "Frequency",
  id_col = "File",
  cond_col = "Condition",
  main_directory = main_directory,
...
theta_wide <- final_dataset_max_theta %>%
  select(File, Condition, Frequency) %>%
  pivot_wider(names_from = Condition, values_from = Frequency, 
              names_prefix = "Theta_Frequency_")

# Load the CCDT dataset
data_ccdt <- read_excel(".xlsx", col_names = TRUE) # * load THRIVE CCDT

# Extract only the numeric subject ID (e.g., "111" from "CCDT_baseline-111-1")
data_ccdt <- data_ccdt %>%
  mutate(File = gsub("CCDT_baseline-(\\d+)-.*", "\\1", File))


# Merge datasets
merged_data <- merge(data_ccdt, theta_wide, by = "File")
...
theta_power_wide <- final_dataset_max_theta %>%
  select(File, Condition, Power) %>%
  pivot_wider(names_from = Condition, values_from = Power, 
              names_prefix = "Theta_Power_")

# Load the CCDT dataset
data_ccdt <- read_excel(".xlsx", col_names = TRUE) # * set your directory


# Extract numeric subject ID from File field
data_ccdt <- data_ccdt %>%
  mutate(File = gsub("CCDT_baseline-(\\d+)-.*", "\\1", File))

# Merge datasets by subject ID
merged_data_power <- merge(data_ccdt, theta_power_wide, by = "File")

# Ensure d-prime is numeric
...
freq_cor_tests <- list(
  ...
  Freq_Ret_cor   = cor.test(merged_data$dprime, merged_data$Theta_Frequency_retr, method = "spearman")
)
# Extract results
...
merged_data <- merged_data %>%
  mutate(
    ...
    Freq_Enc_Ret = Theta_Frequency_encoding - Theta_Frequency_retr,
    Freq_ER_LR   = Theta_Frequency_er - Theta_Frequency_lr,
    Freq_ER_Ret  = Theta_Frequency_er - Theta_Frequency_retr,
    Freq_LR_Ret  = Theta_Frequency_lr - Theta_Frequency_retr
  )
...
### Plot theta peak difference encoding-retrieval ~ d-prime (for visualization now)
...
merged_data$Freq_Enc_Ret <- merged_data$Theta_Frequency_encoding - merged_data$Theta_Frequency_retr
...
ggplot(merged_data, aes(x = Freq_Enc_Ret, y = dprime)) +
  ...
  labs(
    ...
    subtitle = "encoding – retrieval ITF shift vs. d-prime",
    x = "ITF shift (encoding – retrieval, in Hz)",
    y = "d-prime"
  ) +
...
ggplot(merged_data, aes(x = Freq_Enc_Ret, y = dprime)) +
  ...
  labs(
    title = "Correlation between ITF Shift and d-prime",
    subtitle = "Spearman correlation: encoding – retrieval ITF shift vs. d-prime",
    x = "ITF Shift (Encoding – Retrieval, Hz)",
    y = "d-prime"
  ) +
  theme_minimal(base_size = 20) +
  theme(
...
power_cor_tests <- list(
  ...
  Power_Ret_cor   = cor.test(merged_data_power$dprime, merged_data_power$Theta_Power_retr, method = "spearman")
)
# Extract results
...
merged_data_power <- merged_data_power %>%
  mutate(
    ...
    Power_Enc_Ret = Theta_Power_encoding - Theta_Power_retr,
    Power_ER_LR   = Theta_Power_er - Theta_Power_lr,
    Power_ER_Ret  = Theta_Power_er - Theta_Power_retr,
    Power_LR_Ret  = Theta_Power_lr - Theta_Power_retr
  )
...
alpha_freq_wide <- final_dataset_max_alpha %>%
  select(File, Condition, Frequency) %>%
  pivot_wider(names_from = Condition, values_from = Frequency,
              names_prefix = "Alpha_Frequency_")

# Load and clean CCDT data
data_ccdt <- read_excel(".xlsx", col_names = TRUE) # * load THRIVE CCDT
data_ccdt <- data_ccdt %>%
  mutate(File = gsub("CCDT_baseline-(\\d+)-.*", "\\1", File))
...
alpha_freq_cor_tests <- list(
  ...
  Freq_Ret_cor  = cor.test(merged_alpha_freq$dprime, merged_alpha_freq$Alpha_Frequency_retr, method = "spearman")
)
...
merged_alpha_freq <- merged_alpha_freq %>%
  mutate(
    ...
    Freq_Enc_Ret = Alpha_Frequency_encoding - Alpha_Frequency_retr,
    Freq_ER_LR   = Alpha_Frequency_er - Alpha_Frequency_lr,
    Freq_ER_Ret  = Alpha_Frequency_er - Alpha_Frequency_retr,
    Freq_LR_Ret  = Alpha_Frequency_lr - Alpha_Frequency_retr
  )
...
# Convert to wide format
alpha_power_wide <- final_dataset_max_alpha %>%
  select(File, Condition, Power) %>%
  pivot_wider(names_from = Condition, values_from = Power,
...
alpha_power_cor_tests <- list(
  ...
  Power_Ret_cor  = cor.test(merged_alpha_power$dprime, merged_alpha_power$Alpha_Power_retr, method = "spearman")
)
...
merged_alpha_power <- merged_alpha_power %>%
  mutate(
    ...
    Power_Enc_Ret = Alpha_Power_encoding - Alpha_Power_retr,
    Power_ER_LR   = Alpha_Power_er - Alpha_Power_lr,
    Power_ER_Ret  = Alpha_Power_er - Alpha_Power_retr,
    Power_LR_Ret  = Alpha_Power_lr - Alpha_Power_retr
  )
...
### Include ppns with ITF not being 8.446Hz during encoding and retrieval

# Identify participants with 8.446Hz in encoding or retrieval
participants_with_8446_in_enc_or_ret <- final_dataset_max_theta %>%
  filter(Frequency == 8.446, Condition %in% c("encoding", "retr")) %>%
  distinct(File)

# Filter dataset to exclude these participants entirely
final_dataset_no_8446_in_enc_or_ret <- final_dataset_max_theta %>%
...
# Load CCDT dataset
main_directory <- # * set your directory
file_path <- file.path(main_directory, "THRIVE CCDT.xlsx")
data_ccdt <- read_excel(file_path)

# Clean File ID to numeric
data_ccdt <- data_ccdt %>%
  mutate(File = gsub("CCDT_baseline-(\\d+)-.*", "\\1", File))
...
freq_cor_tests <- list(
  ...
  Freq_Ret_cor  = cor.test(merged_data_n20$dprime, merged_data_n20$Theta_Frequency_retr, method = "spearman")
)
...
### Plot together the n=35 and n=20 ITF shift enc-retr ~ dprime
